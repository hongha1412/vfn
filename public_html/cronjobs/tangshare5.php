...
$token = array();
$result = mysql_query("SELECT * FROM tokenpage ORDER BY RAND() LIMIT 0,50");
if ($result) {
...
$res  = mysql_query("SELECT * FROM vipshare ORDER BY RAND() LIMIT 0,100");
while ($post = mysql_fetch_array($res)) {
	
	$get = get_post($post['idfb'], $token[array_rand($token)]);
	if ($get != 0) {
		$slShare  = $get['data'][0]['shares']['count'];
		$id_post = $get['data'][0]['id'];
	} else {
		continue;
	}
	if ($slShare <= $share[$post['goi']]) {
		$post_data = array(
